...
class GameState:
    def __init__(self, board, sheepStat, player_turn):
    ...
    def copy(self):
        ...
        return GameState(self.board.copy(), self.sheepStat.copy(), self.player_turn)
    
    def get_valid_moves(self):
        valid_moves = []
        for x in range(12):
            for y in range(12):
                if self.sheepStat[x][y] > 1:
                    for direction in range(1, 10):
                        if direction == 5:
                            continue
                        if self.is_valid_direction(x, y, direction):
                            for split_group_count in range(1, self.sheepStat[x][y]):
                                valid_moves.append(((x, y), split_group_count, direction))
        return valid_moves
    ...
    def is_valid_direction(self, x, y, direction):
        ...
        nx, ny = x + dx, y + dy
        # Check each step until you hit an obstacle or the edge of the board
        while 0 <= nx < 12 and 0 <= ny < 12:
            if self.board[nx][ny] != 0 or self.sheepStat[nx][ny] > 0:  # If there's an obstacle or sheep
                return False
            nx, ny = nx + dx, ny + dy
        return True  # The direction is valid if no obstacles/sheep are encountered

    def make_move(self, move):
        (x, y), split_group_count, direction = move
        ...
        new_x, new_y = self.calculate_new_position(x, y, direction)
    ...
    def calculate_new_position(self, x, y, direction):
        ...
        while 0 <= nx + dx < 12 and 0 <= ny + dy < 12 and self.board[nx + dx][ny + dy] == 0 and self.sheepStat[nx + dx][ny + dy] == 0:
            nx += dx
            ny += dy
...
class MCTSNode:
    ...
    def backpropagate(self, winner):
        self.visits += 1
        if winner == self.game_state.player_turn:
            self.wins += 1
        if self.parent:
