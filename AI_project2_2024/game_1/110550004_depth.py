...
class GameState:
    ...
    def make_move(self, move):
        ...
        self.board[new_x][new_y] = self.player_turn
...
class MCTSNode:
    ...
    def expand(self):
        ...
        for move in valid_moves:
            ...
            new_game_state.next_player_turn()
            new_node = MCTSNode(new_game_state, self, move)
            self.children.append(new_node)

    def simulate(self, depth_limit):
        depth = 0
        simulated_state = self.game_state.copy()
        while not simulated_state.game_end() and depth < depth_limit:
            depth += 1
...
def run_MCTS(root_state, iterations, depth_limit):
    ...
    for iteration in range(iterations):
        node = root_node
        depth = 0
        while node.children and depth < depth_limit and not node.game_end():
            node = node.select()
            depth += 1
...
def GetStep(playerID, mapStat, sheepStat):
    ...
    best_move = run_MCTS(current_state, 700, 3)
